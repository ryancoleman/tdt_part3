...
class Mol2(object):
  ...
  def blankNew(self):
    ...
    self.atomNum = []
    self.atomName = []
    #all this stuff might need to get optimized if lists suck
  ...
  def copy(self):
    ...
    newM.inputHydrogens = self.inputHydrogens[:]  # and this are mutable
  ...
  def getRMSDclustersAll(self, rmsdCutoff=None, numClusters=1):
    ...
    for rmsdTuple in self.rmsdList:
      ...
      if clusters.different(rmsdTuple[1], rmsdTuple[2]):
        # otherwise already joined
        combine = True
      ...
      for oneBond in xrange(len(self.bondNum)):
        outFile.write(
            "%6d%5d%5d %2s\n" % (
                ...
                self.bondEnd[oneBond],
                string.ljust(self.bondType[oneBond], 2)))
